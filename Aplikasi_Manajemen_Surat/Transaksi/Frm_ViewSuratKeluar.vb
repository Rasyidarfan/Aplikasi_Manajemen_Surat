...
Imports System.IO
Public Class Frm_ViewSuratKeluar
     'Validasi User
  ...
  Sub TampilkanData()
        Try
            ...
            If userRole="Umum" Then
                  Cmd = New SqlCommand("Select A.ID, A.Nomor, A.Tanggal, B.Nama_Tim as Tujuan, A.Perihal, A.Nama_File,     " &
                                 ...
                                 "END AS UploadStatus " &
                                 "From Tran_out A Join Master_Tim B ON A.id_tim=B.ID where A.recid=0  " &
            ...
            ElseIf userRole="Biasa" then
                 Cmd = New SqlCommand("Select A.ID, A.Nomor, A.Tanggal, B.Nama_Tim as Tujuan, A.Perihal, A.Nama_File,     " &
                                 ...
                                 "WHEN A.isupload = 0 THEN 'Need Upload' " &
                                 "END AS UploadStatus " &
                                 "From Tran_out A Join Master_Tim B ON A.id_tim=B.ID where A.recid=0  AND A.Addid =  " & IDlogin &
                                 "Order By A.ID Asc", Conn)
            Reader = Cmd.ExecuteReader
            DgvData.Rows.Clear()
            While Reader.Read
    ...
    Private Sub btn_filter_Click(sender As Object, e As EventArgs) Handles btn_filter.Click
         Try
            ...
            If userRole="Umum" Then
            sqlQuery = "Select A.ID, A.Nomor, A.Tanggal, B.Nama_Tim as Tujuan, A.Perihal, A.Nama_File, " &
                       "Case " &
                       "WHEN A.isupload = 1 THEN 'Uploaded' " &
                       "WHEN A.isupload = 0 THEN 'Need Upload' " &
                       "END AS UploadStatus " &
                       "From Tran_out A Join Master_Tim B ON A.id_tim=B.ID " &
                       "Join Master_Klasifikasi C On A.ID_Klasifikasi = C.KODE where A.recid=0"

            'A.Nomor, A.Tujuan, A.Perihal, A.Nama_File, A.Tahun, B.Nama_Tim, dan C.Kode klasifikasi
            If Not String.IsNullOrEmpty(Txt_search.Text) Then
             sqlQuery += " And (A.Nomor Like '%" & Txt_search.Text & "%' OR A.Tujuan LIKE '%" & Txt_search.Text & "%' OR A.Perihal LIKE '%" &
                    Txt_search.Text & "%' OR A.Nama_File LIKE '%" & Txt_search.Text & "%' OR A.Tahun LIKE '%" & Txt_search.Text & "%' OR B.Nama_Tim LIKE '%" & Txt_search.Text & "%' OR C.KODE LIKE '%" & Txt_search.Text & "%')"
           End If
            ...
            sqlQuery = "Select A.ID, A.Nomor, A.Tanggal, B.Nama_Tim as Tujuan, A.Perihal, A.Nama_File, " &
                       ...
                       "END AS UploadStatus " &
                       "From Tran_out A Join Master_Tim B ON A.id_tim=B.ID " &
                       "Join Master_Klasifikasi C On A.ID_Klasifikasi = C.KODE where A.recid=0 AND A.Addid = " & IDlogin
    ...
    Try
        Using cmd As New SqlCommand("SELECT isupload FROM Tran_out WHERE ID = @DataID", Conn)
    ...
    Private Sub Btn_viewpdf_Click(sender As Object, e As EventArgs) Handles Btn_viewpdf.Click
     ...
     ElseIf MessageBox.Show("Yakin akan View " & DgvData.CurrentRow.Cells(1).Value.ToString & "  ini..?", "Information", MessageBoxButtons.YesNo) = DialogResult.Yes Then
            ...
            Dim dt As New DataTable
             ...
             dt = ExecuteQuery("select path from tran_out where ID=" & DgvData.CurrentRow.Cells(0).Value ) 
           ...
           If CekFilePDFAda(path, DgvData.CurrentRow.Cells(5).Value.ToString) Then 
             With viewpdf
                   .url_pdf = path & "\" &  DgvData.CurrentRow.Cells(5).Value.ToString
                   .name_file =  DgvData.CurrentRow.Cells(5).Value.ToString
                   .lbl_judul.Text = DgvData.CurrentRow.Cells(5).Value.ToString
                   .ShowDialog()
            End With
           Else
    ...
    Private Sub btn_export_Click(sender As Object, e As EventArgs) Handles btn_export.Click
        ...
        ElseIf MessageBox.Show("Yakin akan Export PDF ?", "Information", MessageBoxButtons.YesNo) = DialogResult.Yes Then
           Dim dt As New DataTable
            ...
            End If
             ...
             If CheckUpload(DgvData.CurrentRow.Cells(0).Value) Then
              MessageBox.Show("File Belum Diupload.", "Peringatan", MessageBoxButtons.OK, MessageBoxIcon.Warning)     
                   ...
                   If CekFilePDFAda(path, DgvData.CurrentRow.Cells(5).Value.ToString) Then
                Dim path_namaFile As String =  path & "\" & DgvData.CurrentRow.Cells(5).Value.ToString
                Dim saveFileDialog As New SaveFileDialog()
                saveFileDialog.Filter = "PDF Files|*.pdf"
                saveFileDialog.FileName = DgvData.CurrentRow.Cells(5).Value.ToString
